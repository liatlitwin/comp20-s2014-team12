...
{
    for(var i = 0; i<alg_data["debts"].length; i++)
    {

        var duplicate = false;
        for (var j = 0; j<alg_mapping.length; j++)
        {
            var strpayer = alg_data["debts"][i]["payer"];
        ...
        if(duplicate == false)
        {
            var name = alg_data["debts"][i]["payer"];
    ...
    for(var i = 0; i<alg_data["debts"].length; i++)
    {
        ...
        for (var j = 0; j<alg_mapping.length; j++)
        {
            var strpayto = alg_data["debts"][i]["PayTo"];
            var strtest = alg_mapping[j];
        ...
        if(duplicate == false)
        {
            var name = alg_data["debts"][i]["PayTo"];
            alg_mapping[alg_mapping.length] = name;
        }
    }
...
function createMatrix() // Creates and fills the "alg_matrix" array
{
    ...
    for(var i = 0; i<alg_data["debts"].length; i++)
    {
        var payer = alg_data["debts"][i]["payer"];
        var amount = alg_data["debts"][i]["Amount"];
        var payto = alg_data["debts"][i]["PayTo"];
        var payermap;
        var paytomap;
